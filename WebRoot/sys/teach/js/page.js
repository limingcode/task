		...
		var Method = {
			...
			getData : function(pageIndex){
				 $.getJSON(opts.interFace, function(data){
				    if(data.status == "ok"){
				    	opts.res = data.datas;
				    	if((data.totalNum <= opts.displayCount)&&(data.totalNum > 0)){
				    		...
				    		opts.pNum = data.totalNum/opts.displayCount;
				    	}
