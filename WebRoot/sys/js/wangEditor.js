...
_e(function (E, $) {
    ...
    Txt.fn.bindPasteFilter = function () {
        ...
        var inputState1 = 0;
        var inputState2 = 0;
        $txt.on('paste', 'p,input',function (e) {
            ...
            if ($(this)[0].className == "option") {
                return;
            }
            if (($(this)[0].localName == "input"&&inputState1 == 0)) {
                inputState1 = 1;
                return;
            }
            if ($(this)[0].localName == "p"&&inputState1 == 1){
                inputState1 = 0;
                return;
            }
            if (editor.config.pasteText) {
                ...
                // 拼接为 <p> 标签
                if (pasteHtml) {
                    resultHtml = '<p>' + pasteHtml.replace(/\s+/g," ") + '</p>';
                }
...
_e(function (E, $) {
    ...
    // 编辑器允许的标签 Hlin添加label
    E.config.legalTags = 'p,h1,h2,h3,h4,h5,h6,blockquote,table,ul,ol,pre,label';
...
_e(function (E, $) {
     ...
     E.UI.menus = {
        ...
        img: {
            normal: '<a href="#" tabindex="-1"><i class="wangeditor-menu-img-picture0"></i></a>',
            selected: '.selected'
        },
        video: {
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        // Hlin修改
        var $imgWidth = $('<label>w:<input type="number" style="background-color:#fff;width:4em;text-align:center" ></label>');
        var $alignTop = $('<a href="#"><i class="">上</i></a>');
        var $alignBottom = $('<a href="#"><i class="">下</i></a>');
        var $alignLeft = $('<a href="#"><i class="">左</i></a>');
        var $alignRight = $('<a href="#"><i class="">右</i></a>');
        var $alignLeftInset = $('<a href="#"><i class="wangeditor-menu-img-align-left"></i></a>');
        var $alignCenterInset = $('<a href="#"><i class="wangeditor-menu-img-align-center"></i></a>');
        var $alignRightInset = $('<a href="#"><i class="wangeditor-menu-img-align-right"></i></a>');
        ...
        function render() {
            ...
            $menuContainer.append($delete)
                            ...
                            // .append($noFloat)
                            // .append($floatRight)
                            // .append($alignLeftInset)
                            // .append($alignCenterInset)
                            // .append($alignRightInset)
                            .append($imgWidth)
                            .append($alignTop)
                            .append($alignBottom)
                            .append($alignLeft)
        ...
        function bindToolbarEvent() {
            ...
            $delete.click(function (e) {
                ...
                // 删除图片 Hlin修改 添加父标签删除功能
                commandFn = function () {
                    if ($currentImg.siblings('img').length == 0) {
                        $currentImg.parent("label").remove();
                    }
                    $currentImg.remove();
                };
            ...
            $imgWidth.on('focusout', function(e) {
                var inputVal = $(this).find('input').val();
                if (Number(inputVal) < 20 ){
                    inputVal = 20
                }
                if (Number(inputVal) > 629 ) {
                    inputVal = 629
                }
                $currentImg.css({
                    width:inputVal,
                    height:inputVal / $currentImg.width() * $currentImg.height()
                })
            });
            ...
            // alignLeft
            $alignLeftInset.click(function (e) {
                commandFn = function () {
            ...
            // alignRight
            $alignRightInset.click(function (e) {
                commandFn = function () {
                    // 如果 img 增加了链接，那么 img.parent() 就是 a 标签，设置 align 没用的，因此必须找到 P 父节点来设置 align
                    $currentImg.css({
                        float: 'right'
            ...
            // alignCenter
            $alignCenterInset.click(function (e) {
                commandFn = function () {
                    // 如果 img 增加了链接，那么 img.parent() 就是 a 标签，设置 align 没用的，因此必须找到 P 父节点来设置 align
            ...
            $alignTop.click(function (e) {
                commandFn = function () {
                    if ($currentImg.parents("p").find(".top_img").length) {
                        $currentImg.parents("p").find(".top_img").append($currentImg);
                        $currentImg.parents("p").find("img").each(function(index, el) {
                            $currentImg.parents("p").find(".top_img").append($(this))
                        });
                    }else{
                        $currentImg.wrap('<label class="p_img top_img" contenteditable="false">');
                        $currentImg.parents("p").prepend($currentImg.parent().css({
                            display: "block",
                            width: "100%",
                            textAlign: "center"
                        }));
                    ...
                    $currentImg.parents("p").find(".p_img").each(function(index, el) {
            ...
            $alignBottom.click(function (e) {
                commandFn = function () {
                    if ($currentImg.parents("p").find(".bottom_img").length > 0) {
                        $currentImg.parents("p").find(".bottom_img").append($currentImg);
                        $currentImg.parents("p").find("img").each(function(index, el) {
                            $currentImg.parents("p").find(".bottom_img").append($(this))
                        });
                    }else{
                        $currentImg.wrap('<label class="p_img bottom_img" contenteditable="false">');
                        $currentImg.parents("p").append($currentImg.parent().css({
                            display: "block",
                            width: "100%",
                            textAlign: "center"
                        }));
            ...
            $alignLeft.click(function (e) {
                commandFn = function () {
                    if ($currentImg.parents("p").find(".left_img").length > 0) {
                        $currentImg.parents("p").find(".left_img").append($currentImg);
                        $currentImg.parents("p").find("img").each(function(index, el) {
                            $currentImg.parents("p").find(".left_img").append($(this))
                        });
                    }else{
                        $currentImg.wrap('<label class="p_img left_img" contenteditable="false">');
                        $currentImg.parents("p").prepend($currentImg.parent().css({
                            width: "auto",
                            textAlign: "center",
                            float: "left"
            ...
            $alignRight.click(function (e) {
                commandFn = function () {
                    if ($currentImg.parents("p").find(".right_img").length > 0) {
                        $currentImg.parents("p").find(".right_img").append($currentImg);
                        $currentImg.parents("p").find("img").each(function(index, el) {
                            $currentImg.parents("p").find(".right_img").append($(this))
                        });
                    }else{
                        $currentImg.wrap('<label class="p_img right_img" contenteditable="false">');
                        $currentImg.parents("p").prepend($currentImg.parent().css({
                            textAlign: "center",
                            float: "right"
                        }));
        ...
        function show() {
            ...
            // Hlin添加
            $imgWidth.find('input').val(parseInt($currentImg.width()));
            // disable 菜单 Hlin修改
        ...
        // click img 事件
        $currentTxt.on('mouseup', 'img', function (e) {
            e.preventDefault();
