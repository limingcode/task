    ...
    <div class="wa_box sq_detail_list">
        <div class="wa_table_oper">
            <div  class="table_name">
                <span><a href="<%=path %>/work/scheduleStudent.jhtml">学生进度查询</a></span>
                <span><a href="<%=path%>/work/scheduleStudentDetail.jhtml?studentId=${studentId }&courseId=${courseId }&studentName=${studentName}&sortNo=${sortNo}">课次进度</a></span>
            ...
            <tbody>
            	<c:forEach items="${resultCardInfoList}" var="card">
	            		<tr>
		                    ...
		                    <td><c:if test="${card.questionType!=0 }"> ${card.useTime}s</c:if></td>
		                    <td class="answer">
								<c:choose>
									<c:when test="${card.questionType==5 }"><c:forEach items="${card.answerObj}" var="answer" varStatus="index">
			                            ...
			                            <tbody>
			                            	<c:forEach items="${card.childResultCardInfoList}" var="childCard">
				                                <tr>
								                    ...
								                    <td><c:if test="${childCard.questionType!=0 }"> ${childCard.useTime}s</c:if></td>
								                    <td class="answer" style="width: 234px">
														<c:choose>
															<c:when test="${childCard.questionType==5 }">
															<c:forEach items="${childCard.answerObj}" var="answer" varStatus="index">
															<audio src="http://teaching.skyedu99.com/AppTask/${answer.answer}" controls></audio>
															</c:forEach></c:when>
															<c:otherwise>
															<c:forEach items="${childCard.answerObj}" var="canswer" varStatus="index1">
