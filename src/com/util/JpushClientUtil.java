...
public class JpushClientUtil {
    ...
    private final static String appKey = "988ece7add4294020a7d8f14";
 
    private final static String masterSecret = "194b94d8f345801fd6488d18";
     ...
     * @return 0推送失败，1推送成功
     */
    public static int sendToAliasIos(String alias,String notification_title, String msg_title, String msg_content, String extrasparam) {
        int result = 0;
        try {
            PushPayload pushPayload= JpushClientUtil.buildPushObject_ios_alertWithTitle(alias,notification_title,msg_title,msg_content,extrasparam);
            System.out.println(pushPayload);
            PushResult pushResult=jPushClient.sendPush(pushPayload);
            System.out.println(pushResult);
            if(pushResult.getResponseCode()==200){
     ...
     * @return 0推送失败，1推送成功
     */
    public static int sendToTagsIos(String tag,String notification_title, String msg_title, String msg_content, String extrasparam) {
        int result = 0;
        try {
            PushPayload pushPayload= JpushClientUtil.buildPushObject_ios_alertWithTitleTag(tag,notification_title,msg_title,msg_content,extrasparam);
            System.out.println(pushPayload);
            PushResult pushResult=jPushClient.sendPush(pushPayload);
            System.out.println(pushResult);
            if(pushResult.getResponseCode()==200){
    ...
    private static PushPayload buildPushObject_ios_alertWithTitle(String alias, String notification_title, String msg_title, String msg_content, String extrasparam) {
        System.out.println("----------buildPushObject_ios_registrationId_alertWithTitle");
        return PushPayload.newBuilder()
                //指定要推送的平台，all代表当前应用配置了的所有平台，也可以传android等具体平台
                .setPlatform(Platform.ios())
                ...
                .setNotification(Notification.newBuilder()
                        ...
                        .addPlatformNotification(IosNotification.newBuilder()
                                ...
                                .setSound("sound.caf")
                                //此字段为透传字段，不会显示在通知栏。用户可以通过此字段来做一些定制需求，如特定的key传要指定跳转的页面（value）
                                .addExtra("createDate",DateUtil.getNow())
                                .addExtra("content",msg_content)
                                //此项说明此推送是一个background推送，想了解background看：http://docs.jpush.io/client/ios_tutorials/#ios-7-background-remote-notification
                               // .setContentAvailable(true)
    ...
    private static PushPayload buildPushObject_ios_alertWithTitleTag(String tag, String notification_title, String msg_title, String msg_content, String extrasparam) {
        System.out.println("----------buildPushObject_ios_registrationId_alertWithTitle");
        return PushPayload.newBuilder()
                //指定要推送的平台，all代表当前应用配置了的所有平台，也可以传android等具体平台
                .setPlatform(Platform.ios())
    ...
    public static void main(String[] args){
        JpushClientUtil.sendToAliasIos("hello","testIos1","testIos2","this is a ios Dev test","");
